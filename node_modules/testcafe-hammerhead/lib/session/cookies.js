...
class Cookies {
    ...
    _getCookiesByApi(cookie, urls) {
        const { key, domain, path } = cookie, filters = __rest(cookie, ["key", "domain", "path"]);
        const currentUrls = domain && path ? lodash_1.castArray({ domain, path }) : urls;
        let receivedCookies;
    ...
    deleteCookies(externalCookies, urls = []) {
        ...
        for (const cookie of cookies) {
            const { key, domain, path } = cookie, filters = __rest(cookie, ["key", "domain", "path"]);
            const currentUrls = domain && path ? lodash_1.castArray({ domain, path }) : parsedUrls;
